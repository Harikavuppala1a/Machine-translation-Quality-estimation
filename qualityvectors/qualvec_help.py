  ...
  with variable_scope.variable_scope(
    ...
    with variable_scope.variable_scope("Decoder_For"):
        for i, inp in enumerate(decoder_inputs):
            ...
            inp_concat = tf.concat([inp, context],1)
            print "bvcxz"
            print inp_concat
            print "kkkkkkkk"
            print state
            print tf.concat([inp, context],1)
            print for_cell(inp_concat, state)
    ...
    with variable_scope.variable_scope("Decoder_Back"):
        ...
        for i, (inp, context) in enumerate(zip(reversed(decoder_inputs[2:]), reversed(contexts[:-2]))):
            ...
            cell_output, state = bac_cell(inp_concat, state_size)
...
def embedding_attention_qualvec(
    ...
    # Backward direction cell
        ...
        encoder_outputs, encoder_state_fw, encoder_state_bw = tf.contrib.rnn.static_bidirectional_rnn(for_cell, bac_cell, encoder_inputs, dtype=dtype)
...
def model_with_buckets(encoder_inputs,
  ...
  losses = []
  outputs = []
  print "comment out below to turn off"
  attns = []
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    ...
    for j, bucket in enumerate(buckets):
      ...
      with variable_scope.variable_scope(
        ...
        print "with"
        print bucket[0]
        print len(encoder_inputs[:bucket[0]])
        print len(decoder_inputs[:bucket[1]+1])
        print "fgf"
